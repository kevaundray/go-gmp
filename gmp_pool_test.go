...
// TestModExpBytesPooled tests the convenience function
func TestModExpBytesPooled(t *testing.T) {
	base := []byte{0x02}
	exp := []byte{0x0A}
	mod := []byte{0x03, 0xE8}
	
	result, err := ModExpBytesPooled(base, exp, mod)
	if err != nil {
		t.Fatalf("ModExpBytesPooled failed: %v", err)
	}
	
	expected := []byte{0x18}
	if !bytesEqual(result, expected) {
		t.Errorf("ModExpBytesPooled: expected %x, got %x", expected, result)
	}
	
	// Test error case
	_, err = ModExpBytesPooled(base, exp, []byte{})
	if err != ErrEmptyModulus {
		t.Errorf("Expected ErrEmptyModulus, got %v", err)
	}
}
